...
def build_rewrite_instructions(request: RewriteRequest) -> str:
    ...
    if request.iteration > 1 or request.prior_score > (request.target_score + 0.20):
        escalation = (
            ...
            "- replace abstractions with plain phrasing already present in the text (do NOT add new specifics)\n"
    ...
    return (
        ...
        "- CRITICAL: Do not introduce ANY new factual claims, dates, statistics, laws, regulation names, organization names, "
        "numbers, percentages, dollar amounts, or named examples that are not VERBATIM in the source text. "
        "If a fact is not in the original, do not add it. If you are unsure whether something was in the original, do not add it. "
        "This rule has no exceptions, even if you believe the fact is true.\n"
