...
class PerplexityAdapter:
    ...
    def analyze(self, request: ProviderRequest) -> ProviderResult:
        ...
        output_text = self._chat(model=model, messages=messages, temperature=0.1)
        if not output_text:
        ...
        try:
            ...
            retry_output = self._chat(model=model, messages=retry_messages, temperature=0.0)
            if not retry_output:
    ...
    def review_rewrite(self, request: RewriteReviewRequest) -> RewriteReviewResult:
        ...
        output_text = self._chat(model=model, messages=messages, temperature=0.0)
        if not output_text:
        ...
        for attempt in range(1, attempts + 1):
            try:
                ...
                if exc.response.status_code in {429, 500, 502, 503, 504}:
                    raise ProviderTransientError(f"perplexity request temporarily unavailable: {exc}") from exc
                ...
                if attempt < attempts:
                    ...
                    continue
                raise ProviderTransientError(f"perplexity request temporarily unavailable: {exc}") from exc
        raise ProviderTransientError("perplexity request temporarily unavailable")
